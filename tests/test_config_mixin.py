...
def test_util_mixin_ms_transformer(tmp_path):
    exp = Experiment.from_file(path='./tests/test_configs/ms_transformer.toml')
    assert isinstance(exp.transformer, MultiScaleTransformer)
    assert isinstance(exp.transformer_opt, AdamW)

    assert exp.transformer.n_heads == 8
    assert exp.transformer.emb_dim == 256
    assert exp.transformer.emb_num == 10
    assert exp.transformer.num_scales == 4
    assert exp.transformer.depth == 5
