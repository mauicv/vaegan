...
class ConceptEncodingBlock(nn.Module):
		def __init__(self, m, emb_dim, n_heads, use_bias=True):
			...
			self.cells = Parameter(
				torch.zeros(self.m, self.n_heads, self.head_size)
			)
			self.cells.data.uniform_(-1/self.m, 1/self.m)
			self.q = torch.nn.Linear(emb_dim, emb_dim)
			self.v_weights = Parameter(
				torch.randn(self.m, emb_dim, emb_dim)
			)
			if use_bias:
				self.v_basis = Parameter(
					torch.randn(self.m, emb_dim)
				)
		...
		def forward(self, x):
			...
			if hasattr(self, 'vb'):
				v_ = v_ + self.v_basis.reshape(
					1, self.m, 1, self.n_heads, self.head_size
				) # b, m, l, nh, hs
