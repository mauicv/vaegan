...
#                 disable=not verbose):
#             print(k, seq_inds, min(k - 1, self.block_size - 1))
...
# 
#             #  Here we filter the indices higher than k becuase they shouldn't
#             #  be sampled for this k but actally i think the masking does this
#             #  for us so we can remove this!
#             k_ind = torch.max(torch.min(k, seq_inds[:, level]))
...
class SequentialHierarchySampler:
    ...
    def random_windows(self, xs):
        ...
        inds = torch.randint(0, l - self.block_size, (b, 1), device=self.device)
